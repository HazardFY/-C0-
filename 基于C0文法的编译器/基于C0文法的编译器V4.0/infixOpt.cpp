...
void insertOperands(std::set<string>& varsInBlock, infixNotation notation) {
	if (isVarName(notation.operand1) &&
		varsInBlock.count(notation.operand1) == 0) { // Var not stored, insert it（.cout返回个数）
...
// Store elements from vars in block varsInBlock to varExistenceCount
void checkVarExistenceCount(map<string, int>& varExistenceCount, std::set<string>& varsInBlock) {
	for (std::set<string>::iterator it = varsInBlock.begin();//返回varsBlock中第一个元素迭代器
		it != varsInBlock.end(); ++it) {
		map<string, int>::iterator varIt;//map类型的迭代器varIt
		varIt = varExistenceCount.find(*it);//返回一个指向被查找到元素的迭代器
		if (varIt != varExistenceCount.end()) { // Variable already exists
			varIt->second++;//得带器中的第二个元素，即int ++
		}
