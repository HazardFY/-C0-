...
void nextch() {
	if (charPtr == lineLength ) {//读完整列，选择下一列，否则读取下一个char
		if (infixFile.eof()) {//End Of File 的缩写，表示“文件结束”返回布尔值,当未读到结尾时，返回值为0，读到结尾时，返回值为1
			cout << endl;
		...
		char tempChar;
		//cout << infixFile.peek() << endl;
		while (infixFile.peek() != '\n' && infixFile.peek() != EOF) { // 仍需增加一行字符的长度越界判断  peek() 只是"看"了下一个字符而没有真正读取它
			tempChar = infixFile.get();//获得下一个字符
            //cout << tempChar << endl;
			line[lineLength] = tempChar;
			lineLength++;
		}
		line[lineLength] = infixFile.get(); // Get '\n' or EOF
		lineLength++;
		line[lineLength] = '\0';
		//        cout << endl;
	}
