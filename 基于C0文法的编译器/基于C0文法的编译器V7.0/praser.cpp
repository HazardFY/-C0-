...
void judgement(string &infixString) {
	string logicalLeft = "";
	string logicalRight = "";
	symbol logicalSy = andsy;
	if (sy == lparent) {
		insymbol();
		judgement_child(logicalLeft);
		if (sy == rparent) {
			if (skipFlag) {
				skipFlag = false;
			}
		...
		if (sy == andsy || sy == orsy) {
			...
			switch (logicalSy) {
			case andsy:
				insertInfix("AND", logicalLeft, logicalRight, infixString);
				break;
			case orsy:
				insertInfix("OR", logicalLeft, logicalRight, infixString);
				break;
				}
			}
		...
		judgement_child(infixString);
	}
}
// ＜条件子项＞ ::= ＜表达式＞＜关系运算符＞＜表达式＞｜＜表达式＞
void judgement_child(string &infixString) {
	string compareLeft = "";
	string compareRight = "";
